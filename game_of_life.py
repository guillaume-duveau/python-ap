...
class ensemble_toutes_cellules: # On crée une classe qui permet de "regrouper toutes les cellules" dans le même objet 
    ...
    def __init__(self,matrice): # On munit cette classe d'une méthode qui donne une matrice représentant l'ensemble
    ...
    def liste_indices(position):# On calcule les indices des éléments voisins d'un point d'indice  
        L=[(position[0]+1,position[1]),(position[0]-1,position[1]),(position[0],position[1]-1),(position[0],position[1]+1),]
        L.append((position[0]-1,position[1]-1)) # la case à gauche en haut 
        L.append((position[0]-1,position[1]+1))# la case à gauche en bas 
        L.append((position[0]+1,position[1]+1))# la case à droite en bas 
        L.append((position[0]+1,position[1]-1))# la case à droite en haut
        return L 
    ...
    def matrice_voisinage(ensemble):# On crée la matrice qui compte le nombre de points colorés autour de chaque point
        ...
        for i in range(hauteur):
            for j in range(largeur):
                ...
                M[i][j]=S # On en déduit le nombre de cases noires autour de la case considérée
        return M
...
def mise_a_jour_jeu(screen,matri,i): # l'argument est la "matrice binaire"
    screen.fill(BLANC)
    for j in range(0,len(matri._matrice)): #sur sa hauteur
        for k in range(0,len(matri._matrice[0])): # on itère ensuite  sur la largeur du damier
            if  (matri._matrice[j][k]==1) : # On remplit les cases qui répondent au critère du jeu de vie
            ...
            pg.display.set_caption(f"étape:{i}")
            pg.display.update() # 
